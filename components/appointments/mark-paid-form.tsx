...
import { markPaidAction } from "@/app/(dashboard)/atendimentos/actions";
...
export function MarkPaidForm({ id, date }: { id: string; date: string }) {
  ...
  return (
    <div className="space-y-3 rounded-lg border border-gold/40 bg-gold/5 p-4">
      ...
      <Button
        ...
        onClick={() => method && startTransition(() => markPaidAction(id, method, date))}
      >
